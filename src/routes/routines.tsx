...
routes.post('/', manage, async (c) => {
  try {
    ...
    flash(c, err instanceof Error ? (err.message.toLowerCase().includes('unique') ? 'That plan number already exists.' : err.message) : 'Failed to create routine.', 'err');
  }
...
routes.post('/import', manage, async (c) => {
  ...
  for (const [i, rec] of records.entries()) {
    ...
    try {
      ...
      errors.push(`Row ${rowNo}: ${err instanceof Error ? (err.message.toLowerCase().includes('unique') ? 'duplicate plan_number' : err.message) : 'failed'}`);
    }
  }
